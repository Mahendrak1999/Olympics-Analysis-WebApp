...
st.sidebar.subheader("1896 - 2016")
st.sidebar.image('Olympics-Emblem.png')
...
if user_menu == 'Overall Analysis':
    ...
    with col1:
        ...
        st.title(events)
    with col2:
        st.header("Participated Nations")
        st.title(nations)
    with col3:
        st.header("Participated Athletes")
        st.title(athletes)
...
if user_menu == 'Country-wise Analysis':
    ...
    fig = px.imshow(pt, labels=dict(x="Year", y="Sport", color="Medals"), x=pt.columns, y=pt.index)
...
if user_menu == 'Athlete wise Analysis':
    ...
    # Use sns.scatterplot directly without creating subplots
    fig, ax = plt.subplots(figsize=(10, 6))  # Create a figure and axis objects with a specified size
    sns.scatterplot(data=temp_df, x='Weight', y='Height', hue='Medal', style='Sex', s=60,
                    ax=ax)  # Use the axis object for plotting
    plt.title('Height Vs Weight')
    plt.xlabel('Weight')
    plt.ylabel('Height')
    plt.legend(title='Medal')
    st.pyplot(fig)  # Display the plot using st.pyplot(fig)
